...
private fun parseMap(almanac: List<String>, start:String):Map<Long,Pair<Long,Long>>{
  var map = mutableMapOf<Long,Pair<Long,Long>>()
  var i = 0
...
private fun parseAlmanac1(almanac: List<String> ):List<Long>{
  ...
  if (seeds != null) {
    return seeds.asSequence().map{ seed->getMapping(seed2soil, seed)}
      ...
      .map{fertilizer->getMapping(fertilizer2water,fertilizer)}.map{water->getMapping(water2light,water)}
      .map{light->getMapping(light2temperature,light)}.map{temperature->getMapping(temperature2humidity,temperature)}
...
private fun parseAlmanac2(almanac: List<String> ):Long{
  var seeds = mutableListOf<Pair<Long,Long>>()
